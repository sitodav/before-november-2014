...
void test(){
    	...
    	if(!mousePressed)
    		return;
    	campoVett.azzeraCelle();
    	for(int k=mouseY-100;k<=mouseY+100;k++){
    			if(k<0) continue;
    			if(k>=height) break;
    			for(int f=mouseX-100;f<=mouseX+100;f++){
    				if(f<0) continue;
    				if(f>=width) break;

    				
    				campoVett.yForce[k][f]+=(k-mouseY);
    				campoVett.xForce[k][f]+=(f-mouseX);
...
class Sfera implements Movibile{
	...
	public Sfera(int x,int y,r,g,b,int maxraggio){this.x=x; this.y=y; maxradius=maxraggio; this.r=r; this.g=g; this.b=b;}
...
class Lanciatore extends Barra{
	...
	public void lancia(){
		...
		if(indlastspawn>=maxspawn)
			indlastspawn=0;
		spawnati[indlastspawn]=new Spawned(3,3,this.xc,yc,r,g,b);
	}
...
void setup(){
		...
		//lanciatori[0]=new Lanciatore(75,75,10,375,0,255,0,100);
		//sfere[0]=new Sfera(50,375,0,255,0,40);
		//lanciatori[1]=new Lanciatore(75,75,10,450,0,255,255,100);
		//sfere[1]=new Sfera(50,450,0,255,255,40);
		
		//lanciatori[2]=new Lanciatore(75,75,10,50,120,0,255,100);
		//sfere[2]=new Sfera(50,50,120,0,255,40);
